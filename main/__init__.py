...
#     playlists = keyWordPlaylist(user_search)
#     total_tracks = sum(x[2] for x in playlists)
#     playlists_dict = { k[0]: k[1:] for k in playlists } ### convert to a dict to pass to template. in HTML file, loop through dict to make HTML table
#                                                         ### {uri:name, numSongs} to avoid same name removal during dict creation. Lazy workaround b/c rest of code
#     ### SQLite functions 
...
#     data_entry_userSearch(user_search)

#     totalPlaylists = len(playlists_dict)
...
#     c = conn.cursor()
#     playlists_query = c.execute("SELECT *  FROM playlists_db") ###SQL Query
    
#     playlists_dict = {k[0]:k[1:] for k in playlists_query} ###convert query to dict to see if db query works. it does
#     playlists = [[k, playlists_dict.get(k)] for k in playlists_dict] #create list of playlists
...
#     ## API REQUESTS  
#     all_tracks = get_tracks_from_all_playlists(playlists)
#     uniqueTracks = get_unique_tracks_from_all_tracks(all_tracks)
    
#     ##SQL DB ENTRY
#     data_entry_tracks(uniqueTracks)

#     return jsonify(uniqueTracks[:numOfTracks])
